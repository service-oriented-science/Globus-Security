...
public class MyProxyTest {
    ...
    @Test(expectedExceptions = MyProxyException.class)
    public void testPutGet1() throws Exception {
        myProxy.put(cred, username, password, lifetime);
        myProxy.get(cred, username, "ascbdef", lifetime);
    }

    public void testPutGet2() throws Exception {
        myProxy.put(cred, username, password, lifetime);

        GSSCredential mCred = myProxy.get(cred, username, password, lifetime);
        assertTrue(mCred != null);
        GlobusCredential gCred = ((GlobusGSSCredentialImpl) mCred).getGlobusCredential();
    ...
    public void testDestroy() throws Exception {
        ...
        myProxy.destroy(cred, username, password);

        myProxy.get(cred, username, password, lifetime);
    }

    public void testInfo() throws Exception {
        myProxy.put(cred, username, password, lifetime);

        CredentialInfo info = myProxy.info(cred, username, password);

        assertTrue(info != null);
        assertEquals(cred.getName().toString(), info.getOwner());

        long diff = ((info.getEndTime() - info.getStartTime()) / 1000) - cred.getRemainingLifetime();
    ...
    public void testInfo2() throws Exception {
        ...
        for (CredentialInfo anInfo : info) {
            if (credName1.equals(anInfo.getName())) {
                f1 = true;
                assertEquals(credDesc1, anInfo.getDescription());
                assertEquals(cred.getName().toString(), anInfo.getOwner());
                assertEquals(credRetriever, anInfo.getRetrievers());
            } else if (credName2.equals(anInfo.getName())) {
                f2 = true;
                assertEquals(credDesc2, anInfo.getDescription());
                assertEquals(cred.getName().toString(), anInfo.getOwner());
                assertEquals(credRenewer, anInfo.getRenewers());
            }
        }
    ...
    public void testPutGet3() throws Exception {
        ...
        GSSCredential mCred = myProxy.get(username, password, lifetime);
        assertTrue(mCred != null);
        GlobusCredential gCred = ((GlobusGSSCredentialImpl) mCred).getGlobusCredential();
        assertTrue(gCred != null);
        gCred.verify();
    ...
    public void testChangePassword() throws Exception {
        ...
        myProxy.changePassword(cred, params);
    ...
    public void testStore() throws Exception {
        ...
        myProxy.store(cred, globusCred.getCertificateChain(), new BouncyCastleOpenSSLKey(globusCred.getPrivateKey()),
                storeRequest);
    }
