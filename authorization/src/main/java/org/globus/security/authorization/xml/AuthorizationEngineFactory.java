 ...
 */
public class AuthorizationEngineFactory implements FactoryBean<SecurityDescriptor> {

    private AuthZChain adminAuthzChain;
    private AuthZChain authzChain;

    public SecurityDescriptor getObject() throws Exception {
        SecurityDescriptor sec = new SecurityDescriptor();
        sec.setAdminAuthzChain(adminAuthzChain);
        sec.setAuthzChain(authzChain);
        return sec;
    }

    public Class<SecurityDescriptor> getObjectType() {
