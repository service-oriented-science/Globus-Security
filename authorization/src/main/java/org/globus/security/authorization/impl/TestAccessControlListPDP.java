...
public class TestAccessControlListPDP {
    ...
    // Entity A
    Principal entityA1 = new X500Principal("CN=EA1, OU=bar, O=foo");
    Principal entityA2 = new X500Principal("CN=EA2, OU=bar, O=foo");
    // Entity B
    Principal entityB1 = new X500Principal("CN=EB1, OU=bar, O=foo");
    // Entity C
    Principal entityC1 = new X500Principal("CN=EC1, OU=bar, O=foo");
    Principal entityC2 = new X500Principal("CN=EC2, OU=bar, O=foo");

    // ACL has entity A for access and admin
    Vector<Principal> test1 = new Vector(1);
    // ACL has entity A and entity B for access and entity C for admin
    Vector<Principal> test2Access = new Vector(2);
    Vector<Principal> test2Admin = new Vector(1);
    ...
    // ACL has anonymous and entity A allowed for access, anonymous and entity B for admin

    @BeforeClass
    public void setup() {

        this.test1.add(entityA2);

        this.test2Access.add(entityA1);
        this.test2Access.add(entityB1);
        this.test2Admin.add(entityC2);
    ...
    public void test1() throws Exception {
        ...
        AccessControlListPDP pdp = new AccessControlListPDP(this.test1);

        Decision decision = pdp.canAccess(getRequestor(entityA), null);
    ...
    private RequestEntities getRequestor(Subject subject) {
        ...
        Attribute subjectAttribute =
                new Attribute(AttributeUtil.getPeerSubjectAttrIdentifier(), null, Calendar.getInstance(), null);
