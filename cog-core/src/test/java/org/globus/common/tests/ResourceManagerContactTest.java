...
@Test
public class ResourceManagerContactTest {
    ...
    private void verify(ResourceManagerContact contact,
			...
			String serviceName,
			String dn) {
	assertEquals(hostname, contact.getHostName(), "hostname");
	assertEquals(port, contact.getPortNumber(), "port");
	assertEquals(serviceName, contact.getServiceName(), "service");
	assertEquals(dn, contact.getGlobusDN(), "dn");
    }
}
