...
public class PKITrustManagerFactory extends TrustManagerFactorySpi {
     ...
     * @throws KeyStoreException if the initialization fails.
     */
    @Override
     ...
     * ManagerFactoryParameters as defined by the provider. The provider can then call the specified
     * methods in the ManagerFactoryParameters implementation to obtain the needed information.
     * <p/>
     * This implementation requires X509ProxyCertPathParameters.
     *
     * @param managerFactoryParameters The X509ProxyCertPathParameters which are used to create TrustManagers.
    ...
    protected void engineInit(ManagerFactoryParameters managerFactoryParameters)
        ...
        if (managerFactoryParameters instanceof X509ProxyCertPathParameters) {
            X509ProxyCertPathParameters ptmfp = (X509ProxyCertPathParameters) managerFactoryParameters;
            trustManagers.add(new PKITrustManager(new X509ProxyCertPathValidator(), ptmfp));
        } else {
            throw new InvalidAlgorithmParameterException("Factory cannot accept parameters of type: " +
                    managerFactoryParameters.getClass().getCanonicalName());
        }
    }
