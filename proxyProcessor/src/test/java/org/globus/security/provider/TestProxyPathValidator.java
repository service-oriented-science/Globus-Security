...
public class TestProxyPathValidator {
    ...
    private void validateChain(X509Certificate[] chainCerts,
                               KeyStore keyStore,
                               CertStore certStore,
        ...
        for (int i = 0; i < chainCerts.length; i++) {
            certList.add(chainCerts[i]);
        }

        validateChain(certList, keyStore, certStore, policyStore,
                      expectedIdentity,
                      expectedLimited);
    }

    private void validateChain(List certList,
                               KeyStore keyStore,
                               CertStore certStore,
                               SigningPolicyStore policyStore,
                               X509Certificate expectedIdentity,
    ...
    protected KeyStore getKeyStore(X509Certificate[] certificates)
        ...
        KeyStore keyStore = KeyStore.getInstance("TestKeyStore");
        keyStore.load(null, null);
        if (certificates != null) {
            for (int i = 0; i < certificates.length; i++) {
                keyStore
                    .setCertificateEntry(
                        certificates[i].getSubjectDN().getName(),
                        certificates[i]);
            }
        }
        return keyStore;
    ...
    public void validateRejectLimitedCheck() throws Exception {
        ...
        TestCertParameters certStoreParameters =
            new TestCertParameters(null,
                                   this.crls);

        CertStore certStore =
            CertStore.getInstance("TestCertStore", certStoreParameters);
        TestPolicyStore policyStore =
            new TestPolicyStore((Map)null);
    ...
    public void validateGsi3PathGood() throws Exception {
        ...
        certs.add(goodCertsArr[1]);
        certs.add(goodCertsArr[0]);
        validateChain(certs, keyStore, certStore, policyStore, goodCertsArr[6],
                      false);

        // GSI 3 PC indepedent, GSI 3 PC limited impersonation, EEC, CA
        X509Certificate[] chain =
            new X509Certificate[]{goodCertsArr[29], goodCertsArr[7],
                                  goodCertsArr[1], goodCertsArr[0]};
        validateChain(chain, keyStore, certStore, policyStore, goodCertsArr[29],
                      false);
    ...
    public void validatePathWithRestrictedProxy() throws Exception {
        ...
        chain = new X509Certificate[]{goodCertsArr[8], goodCertsArr[1],
                                      goodCertsArr[0]};
        ...
        Map<String, ProxyPolicyHandler> map =
            new HashMap();
        map.put(policyId, new ProxyPolicyHandler() {
            public void validate(ProxyCertInfo info,
                                 CertPath path,
                                 int index)
        ...
        X509ProxyCertPathValidatorResult result =
            (X509ProxyCertPathValidatorResult)validator
                .engineValidate(path, parameters);
    ...
    public void validatePathBad() throws Exception {
        ...
        CertStore certStore = CertStore.getInstance("TestCertStore", null);
        TestPolicyStore policyStore =
            new TestPolicyStore((Map)null);
        // proxy, CA
        ...
        // user, proxy, CA
        chain = new X509Certificate[]{goodCertsArr[1], goodCertsArr[2],
                                      goodCertsArr[0]};
        validateError(chain, keyStore, certStore, policyStore,
                      "Incorrect certificate path");
    ...
    public void validatePathMixedProxy() throws Exception {
        ...
        chain = new X509Certificate[]{goodCertsArr[36], goodCertsArr[2],
                                      goodCertsArr[1], goodCertsArr[0]};
        validateError(chain, keyStore, certStore, policyStore,
                      "Proxy certificate can only sign another proxy certificate of same type");

        // GSI 2 PC, GSI 3 PC, EEC, CA
        chain = new X509Certificate[]{goodCertsArr[35], goodCertsArr[5],
                                      goodCertsArr[1], goodCertsArr[0]};
        validateError(chain, keyStore, certStore, policyStore,
                      "Proxy certificate can only sign another proxy certificate of same type");
    }
    ...
    public void validatePathProxyPathConstraint() throws Exception {
        ...
        chain = new X509Certificate[]{goodCertsArr[11],
                                      goodCertsArr[1], goodCertsArr[0]};
        validateChain(chain, keyStore, certStore, policyStore, goodCertsArr[1],
                      false);

        // GSI 3 PC, GSI 3 PC pathlen=0, EEC, CA
        chain = new X509Certificate[]{goodCertsArr[32], goodCertsArr[11],
                                      goodCertsArr[1], goodCertsArr[0]};
        validateError(chain, keyStore, certStore, policyStore,
                      "Proxy path length constraint violated");


        // GSI 3 PC, GSI 3 independent PC pathlen=1, EEC, CA
        chain = new X509Certificate[]{goodCertsArr[33], goodCertsArr[12],
                                      goodCertsArr[1], goodCertsArr[0]};
        validateChain(chain, keyStore, certStore, policyStore, goodCertsArr[12],
                      false);
    ...
    public void validatePathCAPathConstraint() throws Exception {
        ...
        KeyStore keyStore =
            getKeyStore(new X509Certificate[]{goodCertsArr[16]});
        ...
        validateChain(chain, keyStore, certStore, policyStore, goodCertsArr[15],
                      false);
...
//                      "ProxyPathValidatorException.FAILURE");
    ...
    public void testKeyUsage() throws Exception {
        ...
        for (int i = 0; i < certsArr.length; i++) {
            certsArr[i] =
                CertificateLoadUtil.loadCertificate(
    ...
    public void testNoBasicConstraintsExtension() throws Exception {
        ...
        certStore = CertStore.getInstance("TestCertStore", parameters);
        validateChain(chain, keyStore, certStore, policyStore, goodCertsArr[1],
                      false);
    }
    ...
    public class TestPolicyStore extends SigningPolicyStore {
        ...
        public TestPolicyStore(SigningPolicyStoreParameters param)
